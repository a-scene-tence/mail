...
### 네이버 IMAP 사전 설정
1. **IMAP/POP3 설정은 PC(데스크톱) 웹 전용** — 모바일 웹 환경설정엔 메뉴가 없다. 모바일이면 브라우저에서 "데스크톱 사이트/PC 버전"으로 전환해 `mail.naver.com` 접속.
2. PC 웹 → **좌하단 환경설정 → 상단 POP3/IMAP 설정 → IMAP/SMTP 사용: ON** 저장.
3. **2025-06-24부터 IMAP/POP3/SMTP 접속에 2단계 인증 + 애플리케이션 비밀번호가 필수** → 네이버ID 2단계 인증 켜고 앱 비밀번호 발급, 로그인 화면엔 **앱 비밀번호** 입력(계정 비밀번호 불가).
3. **로그인이 계속 실패하면** 네이버 보안설정의 **"해외 로그인 차단/해외 IP 차단"을 해제**. Vercel 함수가 미국(US East) 리전에서 IMAP에 접속하므로 해외 차단이 켜져 있으면 인증이 막힌다.

> 로그인 화면은 이제 제공자별 **설정 단계 안내**(IMAP 켜기·앱 비밀번호)와 실패 시 **서버 응답(detail)**·사유(`auth`=자격증명/설정, `connect`=연결 실패)를 함께 표시한다. Vercel Runtime Logs에도 `imap 로그인 실패 @<reason>: <메시지>` 기록.
...
5. 받은편지함 선택 모드: 다중 선택 일괄 삭제, 1개 선택 회신/전달 활성/비활성 확인.
...
- 2026-06-12: M4.1 — 받은편지함 선택 모드(다중 삭제·단일 회신/전달) 검증 항목 추가.
