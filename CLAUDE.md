...
## 변경 이력
- 2026-06-16: M21 — 읽은 메일이 계속 안 읽음(굵게)으로 표시되던 문제 수정. 메일 열기는 `getGmail`(단순 GET)이라 Gmail UNREAD 라벨을 제거하지 않아 서버·클라 모두 읽음 미반영이었음. (1) 서버: `markReadGmail`(`/messages/{id}/modify` removeLabelIds UNREAD + M19 `metaCache` 보정)·`markReadImap`(`messageFlagsAdd \Seen`)·`markRead` 디스패처(`mailbox.ts`) 추가, `api/messages/move.ts`에 `action='read'` 분기로 통합(신규 함수 없음, 12/12 유지). (2) 클라: `mailApi.markRead`, `app/read/page.tsx`가 실제 열람 시(`data.unread`일 때 1회, `useRef` 가드) 모든 `['messages']`·`['message']` 캐시의 `unread=false` 낙관적 갱신 + 백그라운드 `markRead` 호출. 프리페치(hover/long-press)는 `getMessage`만 호출해 읽음 처리 안 됨(실제 열람 시에만). `MailGateway`에 `markRead` 추가.
- 2026-06-16: M20 — 메일 본문 링크 클릭 시 이동 안 되던 문제 수정(`app/read/page.tsx`). HTML 본문 iframe이 `sandbox=""`(전 제약)이라 링크 네비게이션이 모두 막혔던 것을, `sandbox="allow-popups allow-popups-to-escape-sandbox"`로 바꾸고 `srcDoc` 앞에 `<base target="_blank">`+`no-referrer` 메타 주입(`wrapBodyHtml`)해 링크가 새 탭으로 열리게 함. **`allow-scripts`·`allow-same-origin`은 계속 미허용**(이메일 JS 실행·출처/쿠키 접근 차단 유지). 평문 본문은 `linkifyText`(HTML 이스케이프 후 `http(s)`/`www.` URL을 `target=_blank` 링크로)로 자동 링크화. 다른 경로 무영향.
- 2026-06-16: M19 — Gmail 증분 새로고침. `listGmail`(`lib/server/gmail.ts`)이 매 새로고침마다 최신 20개 메타를 통째로 배치 조회하던 것을, **새/만료 id만 배치**하도록 변경. 모듈 레벨 워밍 캐시 `metaCache`(key=`accountId:id`, TTL 3분, 토큰/폴더 캐시와 동일 패턴, 자격증명·본문 미저장, 상한 5000 prune/clear) 추가. id 목록(`/messages?labelIds=`)은 항상 라이브라 멤버십(새 메일/삭제/이동)은 즉시 정확, 캐시는 불변 표시 메타(From/Subject/Date/snippet)만 제공. 비용: 새 메일 없으면 배치 0회(≈id 목록 호출뿐), N개면 N개만. 트레이드오프: 타 기기 읽음 처리한 `unread`가 최대 3분 지연(TTL 만료 시 재배치로 보정). 검색/‘더 보기’/전체뷰 집계·본문조회(`getGmail`)·회신/전달 무영향. IMAP은 connect(~1s) 지배라 ROI 낮아 제외(추후 옵션).
- 2026-06-16: M18 — 새 메일 불러오기 경로 개선. 근본 원인은 새 메일을 받을 **능동적·국소적 경로 부재**(수동 새로고침·당겨서 새로고침 없음, `refetchOnWindowFocus:false` → 복귀해도 갱신 안 됨, 새 메일 보려면 앱 전체 리로드). (1) `components/Providers.tsx` `refetchOnWindowFocus:true`+`refetchOnReconnect:true`(복귀·재연결 시 백그라운드 갱신, staleTime 60s로 과도 재조회 방지, keepPreviousData로 깜빡임 없음). (2) 포커스마다 폴더/계정 호출이 새로 열리지 않게 `foldersQ`(mail·read)·`accountsQ`(mail·compose)에 `staleTime:5분`. (3) `app/mail/page.tsx` 헤더에 **수동 '새로고침' 버튼**(`messagesQ.refetch()`, isFetching 동안 비활성) + 기존 스와이프 터치 핸들러 확장한 **당겨서 새로고침**(최상단 세로 우세 dy>90). (4) `app/compose/page.tsx` 발송 후 `window.location.href`→`router.push('/mail/')`+`invalidateQueries(['messages'])`(앱 재부팅 제거, M15 일관). IMAP 워밍 연결 풀은 동시요청/죽은 연결 위험·Gmail 무효과로 제외(추후 옵션).
